package io.websocket.messagingstompwebsocket;
			...
			public void afterConnected(final StompSession session, StompHeaders connectedHeaders) {
				session.subscribe("/topic/greetings", new StompFrameHandler() {
					...
					public void handleFrame(StompHeaders headers, Object payload) {
						UserResponse response = (UserResponse) payload;
						try {
							assertEquals("Hello, Spring!", response.getContent());
						} catch (Throwable t) {
							failure.set(t);
						} finally {
							session.disconnect();
				...
				try {
					session.send("/app/hello", new UserResponse("Spring"));
				} catch (Throwable t) {
					failure.set(t);
					latch.countDown();
				}
